...
def join():
    ...
    if not result or result["approved"] == 0 or result["banned"] == 1:
        return
...
def send_message(data):
    ...
    c.execute("""
        INSERT INTO messages (id, sender, role, text, time)
        VALUES (?, ?, ?, ?, ?)
    """, (message["id"], message["sender"], message["role"], message["text"], message["time"]))
    conn.commit()
    conn.close()
